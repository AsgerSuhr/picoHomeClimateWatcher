...
class ESP8266(object):
        ...
        if len(cmd_output) > 0:
            if cmd_output[-1].rstrip() == b'ERROR':
                ...
                if timeout == 0:
                    ...
                    if debug:
                        print("Using RX timeout of %i ms" % (timeout))
            ...
            if not okay and cmd_timeout == 0 and debug:
                print("RX-Timeout occured and no 'OK' received!")
        return cmd_output
    ...
    def reset(self, debug=False):
        ...
        while timeout > 0:
            if self.uart.any():
                ...
                if debug:
                    print("RX: %s" % (str(boot_log[-1])))
            sleep(2)
            timeout -= 1
        if debug and timeout == 0:
            print("RTimeout occured while waiting for module to boot!")
    ...
    def connect(self, ssid, psk, debug=False):
        ...
        self._set_command(CMDS_WIFI['CONNECT'], ssid, psk, debug=debug, timeout=20)
